...
if os.environ['PATH_INFO'].endswith('.atom'):
    ...
    for tweet in sorted(all_tweets, key=get_created_at, reverse=True):
        print entry % (tweet['user']['name'], tweet['user']['screen_name'], tweet['user']['screen_name'], tweet['id_str'], get_created_at(tweet).date().isoformat(), tweet['id_str'], get_created_at(tweet).isoformat(), p.parse(tweet['text'].encode('ascii', 'replace')).html)
    print feed_footer
else:
    ...
    if os.environ['REQUEST_METHOD'] == 'POST':
        ...
        if 'append' == fields['action'].value:
            if screen_name not in userprefs.following:
